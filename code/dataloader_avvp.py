...
class LLP_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            Map = np.zeros((10, 25, 25))
            for r in range(10):
                a1 = sample['audio_pseudo_labels'][r]
                b1 = sample['audio_pseudo_labels'][r]
                index_a1 = np.where(a1 == 1)[0]
                index_b1 = np.where(b1 == 1)[0]
                for i in index_a1:
                    for j in index_b1:
                        Map[r, i, j] = 1
            sample['aa_map'] = Map
            ...
            Map = np.zeros((10, 25, 25))
            for r in range(10):
                a1 = sample['visual_pseudo_labels'][r]
                b1 = sample['visual_pseudo_labels'][r]
                index_a1 = np.where(a1 == 1)[0]
                index_b1 = np.where(b1 == 1)[0]
                # print(index_a1, index_b1)
                for i in index_a1:
                    for j in index_b1:
                        Map[r, i, j] = 1
            sample['vv_map'] = Map
            ...
            sample['av_map'] = Map
            sample['va_map'] = np.transpose(Map, (0, 2, 1))
